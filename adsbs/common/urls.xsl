...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- The book family (ds or cds). Needed to use the proper FTP path. -->
  <xsl:param name="family">ds</xsl:param>

  <!-- The libc model used for Hds -->
  <xsl:param name="model" select="'glibc'"/>

  <!-- The kernel series used for Hds -->
  <xsl:param name="kernel" select="'2.6'"/>

  <!-- Do we use a package manager? -->
  <xsl:param name="pkgmngt" select="'n'"/>

  <!-- The system for ds: sysv of systemd -->
