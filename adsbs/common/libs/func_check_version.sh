...
inline_doc
  ...
  write_error_and_die() {
   ...
   # ouput don't follow our expectations
    ...
    echo "but adsbs has failed to detect it, press 'c' and 'ENTER' keys to continue,"
    echo -n "otherwise press 'ENTER' key to stop adsbs.  "
    read ANSWER
    if [ x$ANSWER != "xc" ] ; then
...
#----------------------------#
: << inline_doc
Those tools are needed for the proper operation of adsbs
inline_doc
  ...
  if [ -x $SUDO_LOC ]; then
    ...
    echo "${nl_}\"${RED}sudo${OFF}\" ${BOLD}must be installed on your system for adsbs to run"
    exit 1
  fi
  ...
  if [ -x $WGET_LOC ]; then
    ...
    echo "${nl_}\"${RED}wget${OFF}\" ${BOLD}must be installed on your system for adsbs to run"
    exit 1
  fi
  ...
  # xmllint is unavailable, especially when on recent non-ds hosts.

  XMLLINT_LOC="$(whereis -b xmllint | cut -d" " -f2)"
  XSLTPROC_LOC="$(whereis -b xsltproc | cut -d" " -f2)"

  if [ ! -x $XMLLINT_LOC ]; then
    echo "${nl_}\"${RED}xmllint${OFF}\" ${BOLD}must be installed on your system for adsbs to run"
    exit 1
  fi
  ...
  if [ -x $XSLTPROC_LOC ]; then
    ...
    echo "${nl_}\"${RED}xsltproc${OFF}\" ${BOLD}must be installed on your system for adsbs to run"
    exit 1
  fi
}
...
#----------------------------#
: << inline_doc
In addition to the tools needed for the ds part, docbook-xml
is needed for installing the Bds tools
inline_doc
...
<article>
  ...
  if `echo $XML_FILE | xmllint -nonet -noout -postvalid - 2>/dev/null` ; then
    ...
    echo "Error: you need the Docbook XML DTD for installing Bds tools"
    exit 2
  fi
}
