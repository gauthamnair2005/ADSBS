...
# Make sure that the first argument is a adsbs logs directory
[[ ! -f "$LOGSDIR"/000-masterscript.log ]] && \
  echo -e "\nLooks like $LOGSDIR isn't a adsbs logs directory.\n" && exit
...
# If found, dump adsbs.config file in a readable format
if [[ -f adsbs.config ]] ; then
  echo -e "\n\tadsbs configuration settings:\n" >> "$REPORT"
  cat adsbs.config | sed -e '/parameters/d;s/.\[[013;]*m//g;s/</\t</;s/^\w\{1,6\}:/&\t/' >> "$REPORT"
else
  echo -e "\nNOTE: the adsbs configuration settings are unknown" >> "$REPORT"
fi
...
# root dir, excluding the adsbs directory (and lost+found). We assume
# that the rootdir is $LOGSDIR/../..
DU1=`du -skx --exclude=adsbs --exclude=lost+found --exclude /var/lib $LOGSDIR/../.. | cut -f1`
DU1MB=`perl -e 'printf "%.3f" , ('$DU1' / '1024')';`
INSTALL=`perl -e 'print ('$DU1' - '$DU1PREV')';`
INSTALLMB=`perl -e 'printf "%.3f" , ('$DU1MB' - '$DU1MBPREV')';`
echo -e "Installed files disk usage:\t\t\t\t$INSTALL KB or $INSTALLMB MB\n" >> $REPORT
