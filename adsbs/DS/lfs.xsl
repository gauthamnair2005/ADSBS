...
<!-- Parameters -->
  ...
  <xsl:param name='domain'      select='"ds.org"'/>
  ...
  <xsl:param name='script-root' select="'adsbs'"/>
...
<!-- Only adapted to ds-20170310 and later -->
      ...
      <xsl:when test="contains(string(.),'&lt;ds&gt;')">
        <xsl:value-of select="$hostname"/>
      </xsl:when>
      <xsl:otherwise>
...
# Start of ds book script
</xsl:text>
  </xsl:template>

  <xsl:template name="end-script">
    <xsl:text>
# End of ds book script
