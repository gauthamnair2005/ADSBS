...
#----------------------------#
  ...
  # hds user and group are also presents in the host, and a backup
...
#----------------------------#
  ...
  for file in chapter05/* ; do
    ...
    # Skip this script depending on adsbs.conf flags set.
    case $this_script in
...
#----------------------------#
  ...
  for file in chapter06$N/* ; do
    ...
    # In kernfs we need to set Hds and not to use chroot.
    case "${this_script}" in
...
#----------------------------#
  ...
  for file in chapter07/*; do
    ...
    case $this_script in
      ...
      *console)  continue  ;; # Use the file generated by ds-bootscripts
    ...
    case "${this_script}" in
      *bootscripts*)
        ...
        CHROOT_Unpack $(get_package_tarball_name "ds-bootscripts")
        bds_bootscripts=$(get_package_tarball_name "bds-bootscripts" | sed -e 's/.tar.*//' )
        echo -e "\t@echo \"\$(MOUNT_PT)\$(SRC)/$bds_bootscripts\" >> sources-dir" >> $MKFILE.tmp
        ;;
      *kernel)
        name="linux"
        pkg_tarball=$(get_package_tarball_name $name)
...
#----------------------------#
  ...
  # Add the Bds_TOOL targets, if needed
  [[ "$Bds_TOOL" = "y" ]] && wrt_bds_tool_targets
...
all:	ck_UID mk_SETUP mk_LUSER mk_SUDO mk_CHROOT mk_BOOT create-sbu_du-report mk_CUSTOM_TOOLS mk_Bds_TOOL
	@sudo make do-housekeeping
	@echo "$VERSION - adsbs build" > hds-release && \\
	sudo install -m444 hds-release \$(MOUNT_PT)/etc/hds-release
...
mk_Bds_TOOL: mk_CUSTOM_TOOLS
	@if [ "\$(ADD_Bds_TOOLS)" = "y" ]; then \\
	  \$(call sh_echo_PHASE,Building Bds_TOOL); \\
	  sudo mkdir -p $BUILDDIR$TRACKING_DIR; \\
	  (sudo \$(CHROOT2) "cd \$(SCRIPT_ROOT) && make BREAKPOINT=\$(BREAKPOINT) Bds_TOOL"); \\
	fi;
	@touch \$@
