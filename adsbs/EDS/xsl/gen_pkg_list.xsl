...
<!-- Takes the bds-full.xml file and extract a list
     of packages obeying packdesc.dtd + looks for already
     installed packages in the tracking file (stringparam
     'installed-packages') -->
<!-- Extract also a list of ds packages from stringparam ds-full -->

<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    version="1.0">

  <xsl:param name="ds-full" select="'./ds-xml/ds-full.xml'"/>
  ...
  <xsl:template match="/">
    <princList>
      ...
      <list>
        ...
        <xsl:text>&#xA;    </xsl:text>
        <name>ds Packages</name>
        <xsl:text>&#xA;    </xsl:text>
        <sublist>
          <xsl:attribute name="id">ds-6</xsl:attribute>
          <xsl:text>&#xA;      </xsl:text>
          <name>ds Chapter 6</name>
          <xsl:apply-templates
               select='document($ds-full)//
                            chapter[@id="chapter-building-system"]/
                                 sect1/sect1info'/>
          <xsl:text>&#xA;    </xsl:text>
        </sublist>
        <sublist>
          <xsl:attribute name="id">ds-8</xsl:attribute>
          <xsl:text>&#xA;      </xsl:text>
          <name>ds Chapter 8</name>
          <xsl:apply-templates select='document($ds-full)//chapter[@id="chapter-bootable"]/sect1/sect1info[./productname="kernel"]'/>
          <xsl:text>&#xA;    </xsl:text>
        </sublist>
        <sublist>
          <xsl:attribute name="id">ds-9</xsl:attribute>
          <xsl:text>&#xA;      </xsl:text>
          <name>ds Chapter 9</name>
          <xsl:apply-templates select='document($ds-full)//sect1[@id="ch-finish-theend"]//userinput[starts-with(string(),"echo")]'/>
          <xsl:text>&#xA;    </xsl:text>
        </sublist>
        <xsl:text>&#xA;&#xA;  </xsl:text>
      </list>
<!-- How to have bds-bootscripts versionned? Do not know, so
     avoid it (TODO ?) -->
...
<!-- Only used in ds chapter 9, to retrieve book version -->
    <package>
      <name>ds-Release</name>
      <xsl:element name="version">
        <xsl:copy-of select="substring-after(substring-before(string(),' &gt;'),'echo ')"/>
      </xsl:element>
      <xsl:if
          test="document($installed-packages)//package[name='ds-Release']">
        <xsl:text>&#xA;        </xsl:text>
        <xsl:element name="inst-version">
          <xsl:value-of
            select="document(
                     $installed-packages
                            )//package[name='ds-Release']/version"/>
        </xsl:element>
      </xsl:if>
    </package>
  </xsl:template>
...
<!-- Gcc version is taken from Bds -->
      <xsl:when test="./productname='gcc'"/>
<!-- Shadow version is taken from Bds -->
      <xsl:when test="./productname='shadow'"/>
<!-- Vim version is taken from Bds -->
      <xsl:when test="./productname='vim'"/>
<!-- Dbus version is taken from Bds -->
      <xsl:when test="./productname='dbus'"/>
<!-- Systemd version is taken from Bds -->
