...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="dsbook" select="'ds-full.xml'"/>
...
<!-- Check whether the book is sysv or systemd -->
  ...
  <xsl:include href="ds_make_book.xsl"/>
  ...
  <xsl:template name="apply-list">
    ...
    <xsl:if test="string-length($list) &gt; 0">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:variable name="is-ds">
            <xsl:call-template name="detect-ds">
              <xsl:with-param name="package" select="$list"/>
              <xsl:with-param name="dsbook" select="$dsbook"/>
            </xsl:call-template>
          </xsl:variable>
          <xsl:choose>
            <xsl:when test="$is-ds='true'">
              <xsl:message>
                <xsl:value-of select="$list"/>
                <xsl:text> is an ds package</xsl:text>
              </xsl:message>
              <xsl:call-template name="process-ds">
                <xsl:with-param name="package" select="$list"/>
                <xsl:with-param name="dsbook" select="$dsbook"/>
              </xsl:call-template>
            </xsl:when>
