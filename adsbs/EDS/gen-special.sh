...
Bds_XML=$1
if ! test -f ${Bds_XML}; then
  echo File \`${Bds_XML}\' does not exist
  exit 1
fi
SPECIAL_FILE=$2
if test -z "${SPECIAL_FILE}"; then SPECIAL_FILE=specialCases.xsl;fi
Bds_DIR=$3
if test -z "${Bds_DIR}"; then Bds_DIR=$(cd $(dirname ${Bds_XML})/.. ; pwd);fi

# Packages whose version does not begin with a number
EXCEPTIONS=$(grep 'ENTITY.*version[ ]*"[^0-9"&.].*[0-9]' ${Bds_DIR}/packages.ent |
             sed 's@^[^"]*"\([^"]*\)".*@\1@')

# Non-versioned packages:
NV_LIST="postds-config-profile postds-config-random postds-config-vimrc \
...
cat >tmpfile << EOF
  <xsl:template name="expand-deps">
    ...
    <xsl:choose>
EOF
for file in $(ls ${Bds_DIR}/x/installing/x7* | grep -v x7driver); do
...
for ver_ent in $EXCEPTIONS; do
  id=$(grep 'xreflabel=".*'$ver_ent $Bds_XML | sed 's@.*id="\([^"]*\)".*@\1@')
  [[ -z $id ]] && continue
  cat >>$SPECIAL_FILE << EOF
    <xsl:when test="@id='$id'">
