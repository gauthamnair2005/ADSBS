...
all:	ck_UID mk_SETUP mk_LUSER create-sbu_du-report mk_CUSTOM_TOOLS mk_Bds_TOOL mk_ROOT
...
mk_Bds_TOOL: mk_CUSTOM_TOOLS
	@if [ "\$(ADD_Bds_TOOLS)" = "y" ]; then \\
	  ...
	  (sudo \$(SU_LUSER) "mkdir -p $BUILDDIR$TRACKING_DIR"); \\
	  (sudo \$(SU_LUSER) "source .bashrc && cd \$(MOUNT_PT)/\$(SCRIPT_ROOT) && make BREAKPOINT=\$(BREAKPOINT) Bds_TOOL"); \\
	fi;
	@touch \$@

mk_ROOT: create-sbu_du-report
	@\$(call echo_SU_request)
	@echo "$VERSION-sysroot - adsbs build" > cds-release && \\
