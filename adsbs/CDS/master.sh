...
#--------------------------------------#
  ...
  Cds_HOST="$(echo $MACHTYPE | sed "s/$(echo $MACHTYPE | cut -d- -f2)/cross/")"
(
cat << EOF
...
if ! [ -e final-preps/*variables ]; then
  (
  cat << EOF
	echo "export Cds_HOST=\"${Cds_HOST}\"" >> \$(LUSER_HOME)/.bashrc && \\
	echo "export Cds_TARGET=\"${TARGET}\"" >> \$(LUSER_HOME)/.bashrc && \\
	echo "export Cds_TARGET32=\"${TARGET32}\"" >> \$(LUSER_HOME)/.bashrc && \\
EOF
  ) >> $MKFILE.tmp
fi
(
...
#--------------------------------------#
  ...
  for file in chroot/* ; do
    ...
    # Same for e2fsprogs (in Cds 1.1.0)
...
#--------------------------------------#
  ...
  for file in boot/* ; do
    ...
    case $this_script in
      *kernel)        name=linux                    ;;
      *bootscripts)   name="bootscripts-cross-ds"  ;;
      *boot-scripts)  name="boot-scripts-cross-ds" ;;
      *udev-rules)    name="udev-cross-ds"         ;;
      ...
      *)              name=`echo $this_script | sed -e 's@[0-9]\{3\}-@@' -e 's@-build@@' ` ;;
      ...
      # Identify the unique version naming scheme for the cds bootscripts..(bad boys)
    pkg_tarball=$(get_package_tarball_name $name)
...
#--------------------------------------#
  ...
  if [[ -z "$1" ]] ; then
    ...
    # Remove Bzip2 binaries before make install (Cds-1.0 compatibility)
...
#--------------------------------------#
  ...
  for file in network/* ; do
    ...
    name=`echo $name1 | sed -e 's@-64bit@@' \
                            ...
                            -e 's@n32@@'`
    case $name in
      network-scripts) name=cds-network-scripts ;;
    esac
...
all: ck_UID mk_SETUP mk_F_PREPS mk_SUDO mk_SYSTOOLS create-sbu_du-report mk_CUSTOM_TOOLS mk_Bds_TOOL
	@sudo make do-housekeeping
	@echo "$VERSION - adsbs build" > cds-release && \\
	sudo mv cds-release \$(MOUNT_PT)/etc && \\
	sudo chown root:root \$(MOUNT_PT)/etc/cds-release
...
mk_Bds_TOOL: create-sbu_du-report
	@if [ "\$(ADD_Bds_TOOLS)" = "y" ]; then \\
	  ...
	  (sudo \$(CHROOT1) "make -C $Bds_ROOT/work"); \\
	fi;
	@touch \$@
...
makesys: mk_FINAL mk_CUSTOM_TOOLS mk_Bds_TOOL
	@echo "$VERSION - adsbs build" > /etc/cds-release
	@\$(call echo_finished,$VERSION)
...
mk_Bds_TOOL: mk_FINAL
	@if [ "\$(ADD_Bds_TOOLS)" = "y" ]; then \\
	  \$(call sh_echo_PHASE,Building Bds_TOOL); \\
	  ( make -C $Bds_ROOT/work ); \\
	fi;
	@touch \$@
